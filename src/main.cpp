...
int main() {
    ...
    const int   layerWidth            = 128;           // Hidden layer width
    const int   layerDepth            = 2;             // Number of hidden layers
    ...
    const float learningLossDropout   = 1.5f;          // Stop training at this loss level
    
    // Sampler  
    const float samplerTemp           =  0.7f;         // Temperature  0.7  1.3
    const int   samplerTopK           =  400;          // 0 will disable
    const float samplerTopP           =  0.4f;         // 1.0 will disable
    ...
    const int tokenCountMax           = inputWidth;    // Absolute max number of tokens to sample
    const int sentencesCountMax       = 1;             // Max number of sentences or strings of tokens broken by a period
    ...
    const int feedWidth = 4 * layerWidth;
    uint64_t epoch;
    float avgLoss;
    ...
    while (true) {
        ...
        if (keyboard_string[0] == '/') {
            ...
            if (keyboard_string.rfind("drop", 0) == 0) {
                std::istringstream iss(keyboard_string);
                std::string cmd;
                iss >> cmd;
                iss >> lossDrop;
        ...
        // Encode user prompt
        std::vector<int> userContext = Encode(vocab, keyboard_string, true, false);
        
        for (unsigned int id=0; id < userContext.size(); id++) 
            fullContext.push_back( userContext[id] );
        
        TokenOutputPipeline pipe;
        unsigned int sentence_count = 0;
        unsigned int word_count = 0;
        
        // Generate sentences with basic structural checks
        int generated = 0;
        while (generated < tokenCountMax) {
            
            int token = SampleNextToken(model, fullContext, sampler);
            ...
            fullContext.push_back(token);
            
            std::string word;
            if (PreprocessToken(vocab, token, word)) {
                if (!is_special(word)) {
                    if (is_wordish(word)) word_count++;
                    if (is_sentence_end(word)) sentence_count++;
                    
                    pipe.emit(word);
                    
                    if (sentence_count >= sentencesCountMax) 
                        break;
                }
            }
...
static void TrainModel(std::string& trainingFilename, std::string& modelFilename,
    ...
    while (keepTraining) {
        ...
        for (size_t startIdx = 0; startIdx < inputSequences.size(); startIdx += microBatchSize) {
            ...
            trainer.ApplyGradients(model, accumulatedGradients, /*scale*/1.0f);
            ...
            // Progress toward dropout target (0..100)
