...
// Section directory entry for v2
...
static inline bool StreamWriteLE32(std::ostream& out, uint32_t v) {
    unsigned char b[4];
...
static inline bool StreamReadLE32(std::istream& in, uint32_t& v) {
    unsigned char b[4];
...
static inline bool WriteModelDims(std::ostream& out, const ModelDims& d) {
...
static inline bool ReadModelDims(std::istream& in, ModelDims& d) {
    uint32_t v=0;
...
static inline bool StreamWriteLE64(std::ostream& out, uint64_t v) {
    unsigned char b[8];
    b[0] = (unsigned char)(v & 0xFFu);
    b[1] = (unsigned char)((v >> 8) & 0xFFu);
    b[2] = (unsigned char)((v >> 16) & 0xFFu);
...
static inline bool StreamReadLE64(std::istream& in, uint64_t& v) {
    unsigned char b[8];
...
// Stream model versions
static bool SaveVocabBinaryToStream(const Vocabulary& vocab, std::ostream& out) {
...
static bool LoadVocabBinaryFromStream(Vocabulary& vocab, std::istream& in) {
...
// ===== OPTIMIZER STATE SAVE/LOAD (new) =====
static void EnsureAdamSize(AdamState& s, size_t n) {
    if (s.m.size() != n) s.m.assign(n, 0.0f);
    if (s.v.size() != n) s.v.assign(n, 0.0f);
}

// write: [u32 len][len floats m][len floats v]
static bool WriteAdamState(std::ostream& out, const AdamState& s, size_t expected_len) {
...
static bool ReadAdamState(std::istream& in, AdamState& s, size_t expected_len) {
    uint32_t len = 0u;
    if (!StreamReadLE32(in, len)) return false;
...
static bool SaveModelPackage(const std::string& path,
                             const TransformerLauguageModel& model,
...
static bool LoadModelPackage(const std::string& path,
                             TransformerLauguageModel& model,
                             Vocabulary& vocab,
                             NeuralNetwork& trainer,
                             uint64_t& epoch_out,
                             float& current_lr_out,
                             float& last_loss_out) {
...
static bool UpdateModelLROnDisk(const std::string& modelPath, float newLR,
                                uint64_t* outEpoch = NULL, float* outOldLR = NULL) {
    TransformerLauguageModel  m;
    Vocabulary     v;
