...
public:
    ...
    int  GetNextToken(const TransformerLauguageModel& model, const std::vector<int>& context_ids, const SamplingParams& P);
    
    // Generates a list of the next probable tokens.
    std::vector<TokenCandidate> GetProbableTokens(const TransformerLauguageModel& model, const std::vector<int>& context_ids, 
