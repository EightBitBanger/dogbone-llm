...
public:
    ...
    // Single-program API (kept for backward compatibility)
    ...
    // Named program API (new): build and switch between multiple compute programs
    bool buildComputeFromSourceNamed(const std::string& key, const char* src, const char* debugLabel=nullptr);
    void useNamed(const std::string& key) const;
...
private:
    ...
    // Active program for legacy API
    unsigned program_ = 0;
    // Cache of named programs
    std::unordered_map<std::string, unsigned> programs_;
